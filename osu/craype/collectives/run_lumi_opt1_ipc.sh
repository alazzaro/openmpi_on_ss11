...
#SBATCH -o lumi-slurm-%j-coll_N${NNODES}_opt1_ipc.out
#SBATCH --time=0:30:00
...
for FI_CXI_RX_MATCH_MODE in hybrid; do
    ...
    SUFFIX="_n\${SLURM_NTASKS}_\${FI_CXI_RX_MATCH_MODE}_\${SLURM_JOB_ID}_opt1_ipc"
...
#    if false; then
    (
    	export MPICH_GPU_IPC_THRESHOLD=32768
...
#	CMDS=("osu_alltoall -i 100 -d rocm D D" "osu_allreduce -i 100 -d rocm D D" "osu_allgather -i 100 -d rocm D D" "osu_allreduce -i 100 H H" "osu_alltoall -i 100 H H" "osu_allgather -i 100 H H")
	CMDS=("osu_alltoall -i 100 -d rocm D D" "osu_allreduce -i 100 -d rocm D D" "osu_allgather -i 100 -d rocm D D")
